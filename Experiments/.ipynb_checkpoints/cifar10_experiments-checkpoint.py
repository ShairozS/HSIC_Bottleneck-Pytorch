...
if __name__ == '__main__':
    ...
    for model_name in ['mlp', 'kan']:
        ...
        for init in [torch.nn.init.orthogonal_, torch.nn.init.kaiming_normal_, torch.nn.init.kaiming_uniform_]:
            ...
            for layer_sizes in [[32*32*3, 256], [32*32*3, 512, 256], [32*32*3, 1024, 512, 256], [32*32*3, 256*10]]:
                ...
                for lr in [0.005, 0.0005]:
                    ...
                    for o in ["SGDM", "Adam", "SGD"]:
                        ...
                        if layer_sizes[-1]==256*10 and model_name=='mlp':
                            model_name = "mlpWide"
                        elif layer_sizes[-1]==256*10 and model_name=='kan':
                            continue
                            
                        experiment_name = "CIFAR10_backprop_" + model_name + "_" + str(len(layer_sizes)) + "layers"
                        ...
                        for epoch in range(epochs):
                            ...
                            if epoch % 2 == 0:
                                show_result(trainer, train_loader, test_loader, epoch, logs, device, log_ece = 1)
                                logs[epoch//2].append(end-start)
                        
                        df = pd.DataFrame(logs); df.columns = ['Epoch', 'Train_loss', 'Test_loss', 'Time', 'Train_ECE', 'Test_ECE']; df.head()
                        ...
                        try:
                            df.to_csv(os.path.join("./Results_New", experiment_name + ".csv"))
