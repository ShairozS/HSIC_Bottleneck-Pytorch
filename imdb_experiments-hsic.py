        ...
        for init in [torch.nn.init.kaiming_uniform_]:
            ...
            for layer_sizes in [[100, 32], [100, 64, 32], [100, 128, 64, 32]]:
                ...
                if model_name in ['mlp', 'mlpWide']:
                    model = MLP(layer_sizes = layer_sizes, output_size = 2, dropout = dropout, init = init)
                elif model_name in ['kan']:
                    model = KAN(layer_sizes = layer_sizes, output_size = 2, dropout = dropout, init = init)
                ...
                for lr in [0.0005]:
                    ...
                    for o in ["Adam", "SGD"]:
                        ...
                        experiment_name = "IMDB_hsic_" + model_name + "_" + str(len(layer_sizes)) + "layers"
